...
function getNextFriday(date) {
  ...
  const daysToAdd = 5 - actualDay <= 0 ? 5 - actualDay + 7 : 5 - actualDay;
  const res = new Date(date.setDate(date.getDate() + daysToAdd));
  return res;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startTime = Date.parse(dateStart);
  const endTime = Date.parse(dateEnd);
...
function formatDate(date) {
  ...
  const seconds =
    newDate.getSeconds() < 10
      ...
      : newDate.getSeconds();
  const res = `${newDate.getMonth() + 1}/${newDate.getUTCDate()}/${newDate.getFullYear()}, ${hours}:${minutes}:${seconds}`;
...
function getWeekNumberByDate(date) {
  const newDate = new Date(date);
  const curDate = new Date(date.getFullYear(), 0, 1);
  let res = curDate.getDay() !== 1 ? 1 : 0;
  while (curDate <= newDate) {
    if (curDate.getDay() === 1) {
      res += 1;
    }
...
function getNextFridayThe13th(date) {
  const newDate = new Date(date);
  let res;
  for (let i = newDate.getMonth(); i < 12; i += 1) {
    const nextDate = new Date(newDate.getFullYear(), i, 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    if (work) {
      ...
      res.push(`${day}-${month}-${startDate.getFullYear()}`);
      work -= 1;
    } else {
      off -= 1;
    }
...
function isLeapYear(date) {
  ...
  if (newDate.getFullYear() % 4 === 0 && newDate.getFullYear() % 100 !== 0) {
    return true;
  }
  return false;
}
